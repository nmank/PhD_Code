...
def flag_mean_iteration(data: list, Y0: np.array, weight: float, eps: float = .0000001) -> np.array:
    ...
    for x in data:
        if weight == 'sine':
            ...
            if sinsq < 0:
                sinsq = 0
            al.append(np.max([np.sqrt(sinsq),eps])**(-1/2))
        elif weight == 'cosine':
            cossq = np.trace(Y0.T @ x @ x.T @ Y0)
            if cossq < 0:
                cossq = 0
            al.append(np.max([np.sqrt(cossq),eps])**(-1/2))
            ...
            if cossq < 0:
                cossq = 0
            al.append(np.max([np.sqrt(sinsq)*np.sqrt(cossq), eps])**(-1/2))
        else:
